...
const FormSettingsDetailed = () => {
  ...
  useEffect(() => {
    if (formData) {
      ...
      setWebhookUrl((formData.settings as any)?.webhookUrl || "");
      setMaxResponses((formData.settings as any)?.maxResponses ? String((formData.settings as any).maxResponses) : "");
      setExpiryDate((formData.settings as any)?.expiryDate ? String((formData.settings as any).expiryDate).split("T")[0] : "");
    }
  }, [formData]);
  ...
  const handleUpdateWebhook = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast({ title: "Updated", description: "Webhook URL saved." });
      } else {
        toast({ title: "Error", description: res.message || "Failed to save webhook.", variant: "destructive" });
      }
    } catch (e) {
  ...
  const handleUpdateLimits = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast({ title: "Error", description: res.message || "Failed to save limits.", variant: "destructive" });
      }
    } catch (e) {
      ...
      <div className="flex flex-col gap-8 pb-20">
        ...
        <section className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-8 shadow-sm">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Webhooks & Integrations</h3>
              <p className="text-sm text-gray-500">Forward form responses to external services (Zapier, Make, Slack etc.)</p>
            </div>
          </div>

          <div className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="webhook-url" className="text-base font-semibold">Webhook URL</Label>
              <Input
                id="webhook-url"
                value={webhookUrl}
                ...
                className="h-12 text-base"
              />
              <p className="text-xs text-gray-400">Whenever a response is submitted, a POST request with the JSON payload is dispatched here.</p>
            </div>
            <Button
        ...
        <section className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-8 shadow-sm">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Form Expiry & Limits</h3>
              <p className="text-sm text-gray-500">Automatically close the form based on rules</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-2">
              <Label htmlFor="detailed-max-responses" className="text-base font-semibold">Maximum Responses</Label>
              <Input
                id="detailed-max-responses"
                type="number"
                value={maxResponses}
                ...
                className="h-12 text-base"
              />
              <p className="text-xs text-gray-400">The form closes automatically when reached.</p>
            </div>
            <div className="space-y-2">
              <Label htmlFor="detailed-expiry-date" className="text-base font-semibold">Expiration Date</Label>
              <Input
                id="detailed-expiry-date"
                type="date"
                value={expiryDate}
                onChange={(e) => setExpiryDate(e.target.value)}
                className="h-12 text-base"
              />
              <p className="text-xs text-gray-400">Submissions will be disabled after this day.</p>
            </div>
          </div>
          ...
          <div className="mt-8 flex">
            <Button
              ...
              disabled={loading}
              className="w-full sm:w-auto h-11 px-8 bg-amber-600 hover:bg-amber-700 text-white"
            >
              {loading ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : <Check className="mr-2 h-4 w-4" />}
              Save Rules
            </Button>
