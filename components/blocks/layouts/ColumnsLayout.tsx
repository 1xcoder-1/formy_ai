...
import { Copy, Columns2, GripHorizontal, Trash2Icon, X } from "lucide-react";
...
const blockType: FormBlockType = "ColumnsLayout";
...
const propertiesValidateSchema = z.object({
    columns: z.string().default("2"),
});

export const ColumnsLayoutBlock: ObjectBlockType = {
    blockCategory,
    blockType,
    ...
    canvasComponent: ColumnsLayoutCanvasComponent,
    formComponent: ColumnsLayoutFormComponent,
    propertiesComponent: ColumnsLayoutPropertiesComponent,
};
...
function ColumnsLayoutCanvasComponent({
    blockInstance,
}: {
    ...
    const { columns } = block.attributes;
    ...
    return (
        ...
        text-center w-28 p-1 rounded-b-full shadow-md"
                        ...
                        {!droppable.isOver && childBlocks?.length == 0 ? (
                            ...
                            <div
                                className={`grid grid-cols-1 ${gridCols} gap-4 py-4 px-3 w-full items-start`}
                            >
...
function ColumnsLayoutFormComponent({
    blockInstance,
    handleBlur,
    formErrors,
}: {
...
function ColumnsLayoutPropertiesComponent({
    blockInstance,
}: {
    blockInstance: FormBlockInstance;
}) {
